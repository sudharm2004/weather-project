...
import Currentweathershimmer from "./shimmer/Currentweathershimmer";
  ...
  useEffect(() => {
    const getCurrentWeatherConditions = async () => {
      let response = await fetch(
        `https://dataservice.accuweather.com/currentconditions/v1/${location}?apikey=D4k7HzArNVZLyjZkYkprRzXGqizJo5cG`
      );
    ...
    weather && (
      <div>
        <div className="input flex items-center">
          ...
          <button
            className="bg-white h-10 w-12 rounded-md hover:bg-black hover:text-white"
        ...
        <div className="weather-info bg-white rounded-md m-2 shadow-xl">
          <div className="location flex justify-between p-3 font-serif">
            <span className="mr-5">
              {props.area},{props.administrativeArea}
            </span>
